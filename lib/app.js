...
App.prototype.startLocalServer = function() {
  ...
  options.cache = 0;
  options.root = this.public_folder;

  var server = httpServer.createServer(options);
  server.listen('8080', 'localhost', function() {
    console.log('Server running with ' + options.root + ' as public folder at the following address http://localhost:8080');
  });
};
...
App.prototype.uploadFolder = function() {
  ...
  if (exists_index){
    var schema = this.getCommentSchema();
    this.askToUser(schema, function(err, result){
      if (err) console.log(error(err));
      else api_request.uploadFolder(this.public_folder, result.comment);
    })    
  }
  else {
...
App.prototype.downloadFolder = function() {
  ...
  api_request.downloadFolder(
    this.createStamplayJson.bind(this)
    );
};
